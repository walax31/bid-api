...
test("should return structured response with empty data array via get method.", async ({
  ...
  const response = await client.get(urlEndPoint)
...
test("should return error message and status code of 422 when field data is missing.", async ({
  ...
  const { product_id } = await makeProductUtil(ProductModel, customer1.customer_id);
  const {bid_id} =await makeBidUtil(BidModel,customer2.customer_id,product_id)
