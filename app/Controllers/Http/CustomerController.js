...
class CustomerController {
  async index({ request }) {
    ...
    const customer = await makeCredentialUtil(Customer).getAll(references);

    return { status: 200, error: undefined, data: customern };
  }
  ...
  async store({ request }) {
    ...
    const rules = {
      ...
      rating_descrcustomer_addressption: "required",
